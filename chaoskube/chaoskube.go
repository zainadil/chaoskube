...
type Chaoskube struct {
	...
	// TODO
	ExcludedTimePeriods []util.TimePeriod
...
func New(client kubernetes.Interface, labels, annotations, namespaces labels.Selector, excludedWeekdays []time.Weekday, excludedTimePeriods []util.TimePeriod, timezone *time.Location, logger log.StdLogger, dryRun bool, seed int64) *Chaoskube {
	c := &Chaoskube{
		Client:              client,
		Labels:              labels,
		...
		ExcludedTimePeriods: excludedTimePeriods,
		Timezone:            timezone,
		Logger:              logger,
		DryRun:              dryRun,
		Seed:                seed,
...
func (c *Chaoskube) TerminateVictim() error {
	...
	for _, tp := range c.ExcludedTimePeriods {
		if tp.Includes(c.Now()) {
			c.Logger.Printf(msgTimePeriodExcluded)
			return nil
		}
	}
